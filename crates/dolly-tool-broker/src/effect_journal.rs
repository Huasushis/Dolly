...
//! (ADR 0009 capability effect-intent seam, v1).
//!
...
//!   applies outside the authoritative database. v1 admits exactly one
//!   [`EffectClass`]: writing one JSON-RPC `tools/call` frame to the
//!   installed MCP server's stdio.
...
/// evidence digest (present only on `APPLIED`/`NOT_APPLIED`) is the digest
/// of the deterministic provider-specific evidence that settled the row.
