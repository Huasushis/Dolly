...
// ---------------------------------------------------------------------------
// 10. WP-013B Asset-side media preparation seam: a canonical AVAILABLE
//    `AssetRef` under the exact current lease succeeds; stale, revoked,
//    expired, foreign, non-AVAILABLE, and non-canonical references fail
...
fn prepare_media_succeeds_for_available_asset_with_current_lease() {
    ...
    let asset_id = asset_ref.asset_id.as_str().to_string();
    let lease = service
        .lease(&capability, &asset_id, "instance-a", "channel-send", 60_000)
        .unwrap();

    let prepared = service
        .prepare_media(
            &capability,
            &prepare_request(asset_ref.clone(), MediaKind::Image, Some("image/png"), lease.lease_id.clone()),
        )
        .unwrap();
    assert_eq!(prepared.asset_ref, asset_ref, "echoes the canonical reference");
    assert_eq!(prepared.media_type.as_str(), "image/png", "authoritative detected type");
    assert_eq!(prepared.media_kind, MediaKind::Image, "kind derived from the detected type");
    assert_eq!(prepared.generation, 1, "exact current generation");
    ...
    let again = service
        .prepare_media(
            &capability,
            &prepare_request(asset_ref, MediaKind::Image, Some("image/png"), lease.lease_id),
        )
        .unwrap();
...
fn prepare_media_refuses_revoked_lease_before_reading_bytes() {
    ...
    fs::write(dir.path().join("objects").join(&asset_id), b"tampered").unwrap();
    let err = service
        .prepare_media(
            &capability,
            &prepare_request(asset_ref, MediaKind::Image, Some("image/png"), lease.lease_id),
...
fn prepare_media_refuses_expired_lease() {
    ...
    let asset_id = asset_ref.asset_id.as_str().to_string();
    let lease = service
        .lease(&capability, &asset_id, "instance-a", "channel-send", 1_000)
        .unwrap();
    clock.advance(2_000);
    let err = service
        .prepare_media(
...
fn prepare_media_refuses_foreign_domain_and_owner_leases() {
    ...
    let asset_id = asset_ref.asset_id.as_str().to_string();
    let lease = service
        .lease(&owner, &asset_id, "instance-a", "channel-send", 60_000)
        .unwrap();
...
#[test]
fn prepare_media_refuses_noncanonical_and_mismatched_references() {
    let dir = tempfile::tempdir().unwrap();
    let (mut service, _clock) = service_at(dir.path(), TestClock::new(T0));
    let capability = cap(&service);
    let png = png_bytes(4, 2);
    ...
    // A non-canonical wire form (orientation outside 1..=8) is refused
    // before any store access.
    let mut noncanonical = asset_ref.clone();
    noncanonical.orientation = Some(9);
    let err = service
        .prepare_media(
            &capability,
            &prepare_request(noncanonical, MediaKind::Image, Some("image/png"), lease.lease_id.clone()),
        )
        .unwrap_err();
    assert_eq!(err.code, AssetErrorCode::InvalidRequest, "a non-canonical reference is refused");

    // A stale reference whose recorded byte length no longer matches the
    // durable record is refused without releasing bytes.
    let mut stale = asset_ref.clone();
    stale.byte_length += 1;
    let err = service
        .prepare_media(
            &capability,
            &prepare_request(stale, MediaKind::Image, Some("image/png"), lease.lease_id.clone()),
        )
        .unwrap_err();
    assert_eq!(err.code, AssetErrorCode::NotFound, "a reference not matching the durable record is refused");

    // A forged media-type claim (action override) is refused.
    let err = service
        .prepare_media(
            &capability,
            &prepare_request(asset_ref.clone(), MediaKind::Image, Some("image/jpeg"), lease.lease_id.clone()),
        )
        .unwrap_err();
    ...
    let err = service
        .prepare_media(
            &capability,
            &prepare_request(asset_ref, MediaKind::Audio, Some("image/png"), lease.lease_id),
        )
        .unwrap_err();
...
fn prepare_media_refuses_active_document_content() {
    ...
    let err = service
        .prepare_media(
            &capability,
            &prepare_request(asset_ref, MediaKind::File, Some("application/pdf"), lease.lease_id),
        )
        .unwrap_err();
    assert_eq!(
        err.code,
...
fn prepare_media_refuses_missing_bytes_fail_closed() {
    ...
    let lease = service
        .lease(&capability, &asset_id, "instance-a", "channel-send", 60_000)
        .unwrap();
    fs::remove_file(dir.path().join("objects").join(&asset_id)).unwrap();
    let err = service
        .prepare_media(
            &capability,
            &prepare_request(asset_ref, MediaKind::Image, Some("image/png"), lease.lease_id),
...
fn prepare_media_enforces_the_current_decoded_byte_bound() {
    ...
    let (asset_id, asset_ref) = seed_live_asset(&mut service, &mut clock, "personal", &big, None);
    ...
    let err = service2
        .prepare_media(
            &capability2,
            &prepare_request(asset_ref, MediaKind::File, None, lease.lease_id),
        )
        .unwrap_err();
    assert_eq!(
        err.code,
