...
pub fn reduce(state: &CoreSnapshot, command: &CoreCommand, input: &EnvironmentInput) -> Transition {
    ...
    match command {
        ...
        CoreCommand::IssueLease(c) => {
            ...
            // Frame-bound gate: a persisted Manifest that carries either
            // required frame bound constrains the chosen Extension generation
            // to meet or exceed the Manifest's complete-frame byte and
            // nesting-depth limits. Mirrors the reference machine's
