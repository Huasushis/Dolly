...
The enabled path uses `dolly.media-store/9` inside `dolly.core-state/16`. It
...
| Core state document | `dolly.core-state/16` |
...
`dolly.core-state/15` fails closed with `CORE_STATE_MIGRATION_REQUIRED`; an
operator runs the explicit offline migration described in `core-runtime.md`
Section 7.7 before starting the instance. Version 14 and earlier are rejected
outright.
