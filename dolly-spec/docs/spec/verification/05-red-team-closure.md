...
| Missing durable Tool dispatch authority | `AUTHORIZED`/`DISPATCHED` prose did not select a database owner, closed row, compare-and-set revision, or reopen decision, so a storage writer could resend after an ambiguous commit | one versioned `tool_call_ledger` table in the authoritative instance SQLite database, exact binding/outbound/result digests, commit-before-send permit, pure recovery, corruption stop, and `TST-TOOL-009..013` |
