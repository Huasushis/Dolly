...
function expectedSplit(rows) {
  ...
  for (const questionType of [...groups.keys()].sort(codeUnitCompare)) {
    ...
    list.forEach((entry, index) => output.push({ schemaVersion: "memory-longmemeval-retrieval/split-row-v3", questionId: entry.row.questionId, questionType, splitDigest: entry.digest, split: index < dev ? "development" : "evaluation", caseSha256: entry.row.caseSha256, distinctGoldSessionIds: entry.row.goldSessionIds }));
  }
  return output.sort((a, b) => codeUnitCompare(a.questionId, b.questionId));
}
function pair(left, right) { return codeUnitCompare(left, right) < 0 ? `${left}\0${right}` : `${right}\0${left}`; }
...
function independentTreatment(input, selectedWeights) {
  ...
  for (const conditionId of CONDITIONS) {
    const graph = new Map();
    if (conditionId !== "content") for (const document of documents) { const messages = conditionId === "shuffled-position" ? shuffle(input.question_id, document.sessionId, document.messages) : document.messages; const sessionPairs = new Set(); for (let left = 0; left < messages.length; left += 1) for (let right = left + 1; right < messages.length; right += 1) { if (conditionId !== "recurrence-no-position" && right !== left + 1) continue; for (const lt of new Set(messages[left].tokens)) for (const rt of new Set(messages[right].tokens)) if (lt !== rt) sessionPairs.add(pair(lt, rt)); } for (const edge of sessionPairs) graph.set(edge, (graph.get(edge) ?? 0) + 1); }
    const admitted = new Map([...graph].filter(([, support]) => support >= 2)); const association = new Map(documents.map((document) => [document.sessionId, Number(query.some((qt) => [...document.content].some((ct) => qt !== ct && !query.includes(ct) && admitted.has(pair(qt, ct))))) ]));
    let edgeBytes = 0; for (const [edge, distinctSessions] of [...admitted].sort(([a], [b]) => codeUnitCompare(a, b))) { const zero = edge.indexOf("\0"); edgeBytes += Buffer.byteLength(JSON.stringify({ left: edge.slice(0, zero), right: edge.slice(zero + 1), distinctSessions }) + "\n"); }
...
export function verifyCore(directory, { validateChecksum = true, validateAttestations = false } = {}) {
  try {
    ...
    if (freeze.schemaVersion !== "memory-longmemeval-retrieval/freeze-v3" || freeze.experimentId !== "memory-longmemeval-retrieval-v0" || freeze.experimentVersion !== 3 || !/^[a-z0-9][a-z0-9._-]{0,79}$/u.test(freeze.runId) || !/^[0-9a-f]{40}$/u.test(freeze.sourceCommit) || typeof freeze.dirtyWorktree !== "boolean" || freeze.relevantSourcesClean !== true || freeze.resultComputationStarted !== false || freeze.preregistrationPath !== EXPECTED_SOURCE_FILES[1] || freeze.protocolPath !== EXPECTED_SOURCE_FILES[0] || freeze.artifactContractPath !== EXPECTED_SOURCE_FILES[3] || freeze.datasetPath !== DATASET_PATH || freeze.datasetSha256 !== DATASET_SHA256 || stableJson(freeze.checksumFiles) !== stableJson(EXPECTED_CHECKSUM_FILES)) fail("FREEZE_MISMATCH", "freeze identity");
    ...
    const verifyRankingRows = (rows, splitName, expectedCount, expectedVariants, selectedWeights, selectedHash) => {
      ...
      const seen = new Set(); for (const row of rows) { const fields = ["schemaVersion", "questionId", "caseSha256", "split", "conditionId", "variants", "cost", ...(splitName === "evaluation" ? ["selectedWeightsSha256"] : [])]; closed(row, fields, "RANKING_ROW_SHAPE_MISMATCH", "ranking row"); if (row.schemaVersion !== "memory-longmemeval-retrieval/ranking-v3") fail("RANKING_ROW_SHAPE_MISMATCH", "ranking schema version"); const key = `${row.questionId}\0${row.conditionId}`; if (seen.has(key)) fail("RANKING_ROW_DUPLICATE", key); seen.add(key); const source = sourceById.get(row.questionId); if (!source || row.caseSha256 !== source.caseSha256 || row.split !== splitName || expectedSplitRows.find((candidate) => candidate.questionId === row.questionId).split !== splitName) fail("RANKING_SOURCE_BINDING_MISMATCH", key); if (selectedHash !== undefined && row.selectedWeightsSha256 !== selectedHash) fail("RANKING_SELECTED_WEIGHT_BINDING_MISMATCH", key); let treatment = treatmentByQuestion.get(row.questionId); if (treatment === undefined) { treatment = independentTreatment(source.input, selectedWeights); treatmentByQuestion.set(row.questionId, treatment); } const recomputed = treatment.find((condition) => condition.conditionId === row.conditionId); if (!recomputed) fail("RANKING_CONDITION_MISMATCH", key); if (row.variants.length !== expectedVariants(row.conditionId)) fail("RANKING_VARIANT_COVERAGE_MISMATCH", key); compareStable(row.variants, recomputed.variants, "RANKING_RECOMPUTATION_MISMATCH", key); closed(row.cost, ["buildMilliseconds", "queryMilliseconds", "edgeCount", "edgeBytes", "corpusRawSessionBytes"], "RANKING_COST_SHAPE_MISMATCH", "cost"); nonnegative(row.cost.buildMilliseconds, "RANKING_COST_INVALID", "build"); nonnegative(row.cost.queryMilliseconds, "RANKING_COST_INVALID", "query"); compareStable({ edgeCount: row.cost.edgeCount, edgeBytes: row.cost.edgeBytes, corpusRawSessionBytes: row.cost.corpusRawSessionBytes }, recomputed.exactCost, "RANKING_COST_MISMATCH", key); }
    };
    verifyRankingRows(development, "development", 588, (condition) => condition === "content" ? 1 : 4, undefined, undefined);
    closed(selected, ["schemaVersion", "developmentRankingsSha256", "selectedWeights", "selections"], "SELECTED_WEIGHT_MISMATCH", "selected weights"); if (selected.schemaVersion !== "memory-longmemeval-retrieval/selected-weights-v3") fail("SELECTED_WEIGHT_MISMATCH", "selected schema version");
    ...
    if (manifest.schemaVersion !== "memory-longmemeval-retrieval/manifest-v3" || manifest.attemptStatus !== "complete" || manifest.experimentId !== "memory-longmemeval-retrieval-v0" || manifest.experimentVersion !== 3 || manifest.runId !== freeze.runId || manifest.preregistrationSha256 !== freeze.preregistrationSha256 || manifest.protocolSha256 !== freeze.protocolSha256 || manifest.datasetSha256 !== DATASET_SHA256 || manifest.sourceCommit !== freeze.sourceCommit || manifest.dirtyWorktree !== freeze.dirtyWorktree || manifest.relevantSourcesClean !== true || manifest.backend !== "local-deterministic-retrieval" || manifest.modelEndpointCapabilityProfile !== null || manifest.modelIdentifier !== null || manifest.executionOrder !== "UTF-16 code-unit question ID, frozen condition order" || manifest.failure !== null) fail("MANIFEST_MISMATCH", "manifest identity");
...
function independentAnalysis(evaluation, development, sourceById, selectedWeights, selectedHash, evalHash) {
  ...
  return { schemaVersion: "memory-longmemeval-retrieval/analysis-v3", selectedWeights, developmentConditionMetrics: aggregateMetrics(development, sourceById, selectedWeights), conditionMetrics, contrasts, knowledgeUpdate: { contentTop1Error: contentError, repeatedPositionTop1Error: positionError, difference: positionError - contentError }, cost: { repeatedPositionEdgeToRawRatioP95: ratioP95, descriptiveByTreatment }, gates: { usefulEffect: useful, knowledgeUpdate: knowledgePass, deterministicCost: costPass }, classification: useful && knowledgePass && costPass ? "supported" : "rejected", selectedWeightsSha256: selectedHash, evaluationRankingsSha256: evalHash };
}

export function validationArtifact(result) { return { schemaVersion: "memory-longmemeval-retrieval/validation-v3", verifierSha256: sha256(readFileSync(fileURLToPath(import.meta.url))), valid: result.valid, classification: result.classification, primaryCode: result.primaryCode, errors: result.errors }; }
